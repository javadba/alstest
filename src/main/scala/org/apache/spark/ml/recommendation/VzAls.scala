...
import org.apache.spark.ml.recommendation.{ALS => MlALS}
...
class VzALS(override val uid: String) extends Estimator[ALSModel] with ALSParams
  ...
  override def fit(dataset: DataFrame): ALSModel = {
    ...
    val (userFactors, itemFactors) = VzALS.train(ratings, rank = $(rank),
 ...
 */
@DeveloperApi
object VzALS extends DefaultParamsReadable[ALS] with Logging {
